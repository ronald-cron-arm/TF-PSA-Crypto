...
class CryptoConfigTool(config_common.ConfigTool):
    """Command line TF PSA Crypto config file manipulation tool."""

    def __init__(self):
        super().__init__(CryptoConfigFile.default_path[0], single_config=False)
        configfiles = [CryptoConfigFile(file) for file in self.args.file]
        self.config = CryptoConfig(*configfiles)
...
if __name__ == '__main__':
    sys.exit(CryptoConfigTool().main())
